...
class ServerWorkerTest(unittest.TestCase):
    ...
    def test_connect_parses_the_message(self):
        self.worker.connect_from_client(['random', 'client_id', 'server_id', 'nested'])
        self.assertEqual(self.worker.main_random_message, 'random')
        self.assertEqual(self.worker.main_client_id, 'client_id')
        self.assertEqual(self.worker.main_server_id, 'server_id')

    def test_connect_from_client_for_valid_input_outputs_a_5_element_output(self):
        output = self.worker.connect_from_client(('random', 'client_id', self.server_id, 'nested'))
    ...
    def test_connect_outputs_error_signal_if_server_id_does_not_match(self):
        output = self.worker.connect_from_client(('random', 'client_id', 'wrong_id', 'nested'))
    ...
    def test_connect_from_client_for_valid_input_5th_section_can_be_decrypted_using_server_key_to_4_element_array(self):
        ...
        encrypted = self.worker.connect_from_client(('random', 'client_id', 'server_id_123', 'nested'))
    ...
    def test_connect_from_trusted_server_unpacks_message_from_trusted(self):
        ...
        self.worker.connect_from_trusted(('random_message', 'encrypted_client_message', nested))
    ...
    def test_if_nonce_does_not_match_returns_error_signal(self):
        ...
        self.worker.nonce = 'server_nonce'
        output = self.worker.connect_from_trusted(('random_message', 'encrypted_client_message', nested))
        self.assertEqual(output, self.worker.error_signal)
